...
### Changed
- DTCG alignment Phase 2d (plans/dtcg-alignment/plan.md): the conflated `transition` token type split into its DTCG successors (decision D5). `ms` timings are spec `duration` `{value, unit}` values (a zero drops its unit — `transition-delay: 0`), and easing curves are spec `cubicBezier` `[x1,y1,x2,y2]` coordinate tuples serialized at two decimals (`cubic-bezier(0.38, 1.21, 0.22, 1.00)` — the transition module self-checks each structured value round-trips to its pinned string). The CSS surfaces DTCG can't type became proprietary: property-name lists are `transitionProperty`, and the `<easing-function>` keyword surface (`ease-out`, …) is the new `transitionTimingFunction` type (registry D4) — DTCG's `cubicBezier` only expresses explicit curves. The reduced-motion a11y modifier followed the durations (`a11yMap` now keys on `duration`); the timing-function compatibility group pairs `cubicBezier` ↔ `transitionTimingFunction` so a keyword slot can be overridden by a curve reference. Applied by a checked-in codemod (`scripts/codemod-2d-transitions.ts`, deleted at the end of the migration) across 9 component modules (structuring `ms`/`0` literals, retyping references and keywords) and 19 theme documents (retyped to the base entry's split type from the default-theme oracle; override `$value`s stay raw strings); the compatibility map, migration table, editor schemas/syntax, and `allowed-token-types.json` regenerate from the enum. Emitted CSS is byte-identical to the golden baseline
- DTCG alignment Phase 2c (plans/dtcg-alignment/plan.md): shadow values are structured DTCG shadows and the legacy `boxShadow` type is gone (decision D5). The elevation ramp's ten entries author arrays of spec `{color, offsetX, offsetY, blur, spread, inset?}` layers (the module self-checks that each round-trips to the exact CSS string the golden baseline pins); `box-shadow: none` is the empty array `[]`, so every shadow token is uniformly `$type: "shadow"`. A central `serializeShadowValue` renders each layer byte-for-byte — offsets drop the unit at zero magnitude (`0`, not `0px`) and colors use CSS Color 4 space notation (`rgb(0 0 0 / 0.05)`), the form the ramp has always emitted. The 38 component shadow slots (all `none`) migrated to `[]` and the one `{elevation.sm}` alias kept its reference; theme override documents keep raw string shadow values per the Phase 2a policy. Applied by a checked-in codemod (`scripts/codemod-2c-shadows.ts`, deleted at the end of the migration) across 8 component modules and 19 theme documents; the reference-compatibility map, migration table, editor schemas (`boxShadowValue` → `shadowValue`), and `allowed-token-types.json` (21 types) regenerate from the enum. Emitted CSS is byte-identical to the golden baseline
- DTCG alignment Phase 2b step 3 (plans/dtcg-alignment/plan.md): color values are structured DTCG colors (decisions D5/D8). The palette module's 278 entries carry spec `{colorSpace, components, alpha?, hex?}` values (`hsl` ramps; `srgb` + hex-fallback globals; `global-transparent` is `srgb` 0,0,0 at alpha 0), validated by a central `colorValueSchema` and serialized by `serializeColorValue` — byte-driven rules: fully-transparent black emits the `transparent` keyword, a `hex` fallback wins at full alpha, `hsl` keeps the legacy comma notation the palette has always emitted, everything else falls back to `rgb()`/`color()` notation. The module self-checks that each global's structured value round-trips to the exact literal the generated SCSS emits. The seven component `"transparent"` literals (pagination ×6, tooltip ×1) became `{color.global-transparent}` aliases — the one deliberate byte change of the phase: those vars now emit `var(--zbk-color-global-transparent)` (computed-value identical; audited diff showed exactly those substitutions across the five affected artifacts) and the golden baseline was re-snapshotted with determinism verified. Theme override documents keep raw string color values per the Phase 2a policy (document conformance is Phase 3's problem)
- DTCG alignment Phase 2b step 2 (plans/dtcg-alignment/plan.md): the primitive palette inverted into token space (decision D7). The palette's source of truth is now a real token module (`src/tokens/colors/palette/tokens/`, key `color`): a generative definition — per-family hue/saturation, one shared lightness ramp, three globals — materializes 278 `color` tokens (`{color.red-500}` targets are now closed-world; the static `paletteMap` allowlist and the converter's virtual-target fallback are deleted, completing invariant I5's palette half). The palette SCSS flipped from hand-written to generated: `npm run generate:palette` writes the `primitiveColor` mixin, all 25 family partials, and `palette/styles.scss` from the definition (drift-guarded by `check:palette` in `npm run check`), preserving the emitted CSS exactly — unlayered var-composed `hsl()` ramps with runtime `--zbk-color-<family>-hue`/`-saturation` knobs. The module is emission-external (`cssEmission = "external"` export / `_cssEmission` snapshot sidecar): the converter never emits it (dev and installed-CLI builds both), overlays exclude it, `zebkit pull` doesn't materialize it, entry-level theme overrides are rejected with a pointer at the runtime hue/saturation knobs, and the editor build skips its override schema while feeding its entries to reference autocomplete examples and the VS Code CSS custom data. The docs primitive-palette data now derives from the token snapshot instead of regex-parsing SCSS (byte-identical output), and the smart-mode global-colors block derives from the same definition. Emitted CSS is byte-identical to the golden baseline
- DTCG alignment Phase 2b step 1 (plans/dtcg-alignment/plan.md): the `borderColor` token type collapsed into `color` (decision D5). It was registry-only — no token entry anywhere carried it — so the change is pure surface shrink: dropped from the type enum, the reference-compatibility map, the converter's palette-reference check, and the editor syntax map; `allowed-token-types.json` regenerates at 21 types. Emitted CSS is byte-identical to the golden baseline
- DTCG alignment Phase 2a step 4 (plans/dtcg-alignment/plan.md): the D5 dimension-type collapse. The eight legacy length-family `$type` names — `spacing`, `sizing`, `rootSize`, `borderWidth`, `borderRadius`, `fontSize`, `rootFontSize`, `letterSpacing` — are gone; structured `{value, unit}` px/rem values are spec `dimension`, every other CSS length surface (`%`, `ch`, `em`, keywords, unitless `0`, `calc()`/`clamp()` expressions) is proprietary `cssDimension`, and references carry their transitively-resolved target's type (a checked-in codemod, `scripts/codemod-d5-collapse.ts`, retyped 1262 entries across 25 source modules and 96 theme files). The fluid-scale resolvers stopped keying on `$type`: the space scale resolves any concrete value (structured floor or theme-substituted raw string) and passes `{…}` references through; the type scale treats entries with a `scale.index` extension or a pinned value as steps; both re-emit resolved entries as `cssDimension` and the converter's legacy verbatim special-case is deleted. The five letter-spacing `a11y: true` opt-ins materialized to the explicit modifier variable (`a11yMap` now only covers the not-yet-migrated `lineHeight`/`transition` families); utility manifests rebound their `types` filters to `["dimension"]` (the margin/padding vs. sizing purpose split already lives in `pattern.exclude`/explicit value lists — regenerated utility SCSS is byte-identical); the reference-compatibility map collapsed to a symmetric `dimension` ↔ `cssDimension` pairing; editor schemas and `allowed-token-types.json` (30 → 22 types) regenerate from the shrunken enum. Emitted CSS is byte-identical to the golden baseline
