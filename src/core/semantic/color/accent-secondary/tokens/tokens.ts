 ...
 *
 * Semantic, role-based knobs for the secondary accent palette. They reference the
 * accent-secondary *scale* (accent-secondary-50..950) rather than raw primitives, so
 * re-skinning the ramp cascades automatically. Reach for these (e.g. `ink-accent-secondary`,
 * `canvas-accent-secondary-muted`) by default; drop to the numeric scale only for a shade the
