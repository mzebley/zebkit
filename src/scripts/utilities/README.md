...
- `types: ["dimension"]` restricts valid values to token keys whose declared `$type` is in the list — a guard against binding a non-length token (or a `cssDimension` keyword like `auto`) into a length family. Since the D5 collapse the whole dimension family shares one `$type`, so purpose splits within a group (margin/padding values vs. width/flex-basis sizes in the spacing group) are carried by the manifests themselves: `pattern.exclude` for derived families, explicit `values` for limiter families. Values mapped in `valueMap` are literal CSS and exempt from the check.
