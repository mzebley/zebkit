...
Each family lives in `src/tokens/semantic/color/{family}/tokens/`, exports its logical `key`, and uses `shared-token-schema.ts` for the common color-role surface. Discovery is automatic; adding a family does not require compiler wiring. Follow the [token-module guide](../../README.md) for the module contract and the [manifesto](../../../../VISION.md#the-token-strata-a-theory-of-change) for the alias-to-primitive rule.
