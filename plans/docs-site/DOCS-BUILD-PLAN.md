...
- **Point** `zebkit.docs.config.json` at `basePreset: "default"`, `tokenPath: "./theme/zebkit-docs"`, `themeName: "zebkit-docs"`, and emit the compiled CSS into `docs/static/zebkit/`.
...
- Hero themes are now config-driven **overlay themes**: each is an entry in the `overlays` array of `zebkit.docs.config.json` (`tokenPath: "./theme/zebkit-hero-<name>"`, `themeName: "<name>"`, `destinationPath: "./docs/static/zebkit/themes"`). The overlay build emits a minimal, scoped `zbk-<name>.css` (selector defaults to `[data-zbk-theme="<name>"]`); the standard `build:tokens` step compiles all of them. `build:hero-themes` now only generates the token-diff manifest.
