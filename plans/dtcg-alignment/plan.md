...
| D4 | Proprietary `$type` registry | Kept, minimized, and centrally defined next to the DTCG set: `display`, `cursor`, `textTransform`, `textDecoration`, `textAlignment`, `fontStyle`, `transform`, `transitionProperty`, `content`, `flex`, `utility`, `asset`, `boolean`, `cssDimension` (CSS sizing values DTCG's `dimension` can't express: `%`, `ch`, `em`, `calc()`, the sizing keywords `auto`/`none`, and unitless `0` — anything valid where a length is expected but not a px/rem `{value, unit}` pair). Everything else moves to a spec type |
