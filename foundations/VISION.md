...
- **The interchange format is the Zebkit DTCG 2025.10 profile.** Full exports, theme overrides, and docs data follow the [Design Tokens Format Module 2025.10](https://www.designtokens.org/TR/2025.10/format/) shape — `$value`/`$type`/`$description`, with zebkit metadata under `$extensions["dev.zebkit"]` and proprietary `$type`s where the standard has no complete CSS equivalent. Aliases use document-root curly references at arbitrary depth (`{group.nested.token}`); `$ref` and `$extends` are deliberately rejected by the Zebkit reader with an actionable error. Enable strict export for one validated, combined, reference-closed DTCG 2025.10 document containing only the standard types Zebkit fully supports; use the full profile when the CSS surface is broader than DTCG.
